...
error_reporting(0);
//--------
$token = "5808198667:AAG_Rg_BNh7luBQEahBC-R0HBKjZNRfwC0Y"; # Token
$admin =5896443755;
$sudo = array("$admin","5896443755");
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"عزيزتي المبرمجه همس تم بنجاح تفعيل البوت ✓
...
bot("sendmessage",[
"chat_id"=>$chat_id,
"text"=>"عزيزتي المبرمجه همس تم بنجاح تعطيل البوت Ж ",
]);
...
if($message and $baageel =="off" and $chat_id != $XSOUdoJ ){
 bot("sendmessage",[
 "chat_id"=>$from_id,
 "text"=>"*للأسف الشديد قامت المبرمجه همس بتعطيل البوت Ж*
",'parse_mode'=>"MARKDOWN",
'disable_web_page_preview' =>true,
...
✅|في بوت الإعلانات 
💯| كل ماعليك فعله ارسال اعلانك وسيتم القبول او الرفض من الادمن
...
'reply_markup'=>json_encode([ 
'inline_keyboard'=>[
[['text'=>'- قناة البوت.','url'=>"t.me/tmoilhms"]],
[['text'=>'- مجموعة البوت.','url'=>"t.me/hms_03"]],
[['text'=>'- مطور البوت.','url'=>"t.me/hms_01"]],
] 
   ])
    ]);
}
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"✅ تم ارسال اعلانك الى الادمن قم بانتظار الموافقة او الرفض  
"
,'reply_to_message_id'=>$message_id,
...
[['text'=>"قبول  ",'callback_data'=>"yesa3 $rand"],
['text'=>"رفض   ",'callback_data'=>"noa3 $rand"]],
[['text'=>'- حظر .' ,'callback_data'=>"/ban @$chat_id"]],
[['text'=>'- صاحب الرساله .','url'=>"tg://user?id=$chat_id"]]
...
$node_id = $update->inline_query->from->id;
$msms = strtolower($update->inline_query->from->username); 
...
$name = bot("getChat",['chat_id'=>$d])->result->first_name;
bot('sendmessage',[
"chat_id"=>$ch,
"text"=>"$txt
...
$hmsban = json_decode(file_get_contents("hmsban.json"), true);
if(preg_match('/ban @/', $data)) { 
$ex = explode('/ban @',$data)[1];
if(!in_array($ex, $hmsban["hmsban"])){ 
$hmsban["hmsban"][] = $ex;
file_put_contents("hmsban.json", json_encode($hmsban,128|64|256));
bot('editMessageText',[
'chat_id'=>$chat_id,
'text'=>"تم الحظر",
'message_id'=>$massge_id,
]);
} else {
bot('editMessageText',[
'chat_id'=>$chat_id,
